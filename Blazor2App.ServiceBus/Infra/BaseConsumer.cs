...
namespace Blazor2App.ServiceBus.Infra
{
    public abstract class BaseConsumer
    {
        ...
        public BaseConsumer(ILogger<BaseConsumer> logger, IServiceContext serviceContext)
        {
            _logger = logger;
            _context = serviceContext;
        }

        public Task Consume<T>(ConsumeContext<T> context, Func<Task> func, string startEvent, string okEvent, string errorEvent, object eventProperties) where T : class
        {
            _context.CorrelationId = context.CorrelationId.Value;
